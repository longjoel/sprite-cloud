...
The pre-beta `v0.3.x` notes below described an abandoned desktop/AppImage
prototype and are retained only as project history. The current beta is the
browser player plus Linux `sc-server` host runtime; there is no supported
Tauri desktop client, AppImage download, or XMB shell.
...
- Per-server core override system: dashboard users can change which libretro core
  ...
  - sc-server resolves overrides via `core_for_platform()`, stored in server metadata
- Public IP detection in sc-server (ipify.org) displayed in dashboard
...
- **Audio**: capped native sample rate to 48 kHz in sc-core
  - SameBoy outputs 2.1 MHz audio; GStreamer's audioresample choked on 43:1 ratios
  - Bucket-average decimator in sc-core reduces to 48 kHz before shared memory
...
- sc-core binary now deployed alongside sc-server on VAULT

### Fixed

- 56 stale game sessions cleaned up (development cruft)
- Phantom sc-server instance on VPS stealing commands from real VAULT server
...
- DB-backed email/password authentication with protected bootstrap invitation and invite-only enrollment
  - `/api/auth/setup` and `/api/auth/signup` are retired and return `410`
  - First admin uses the one-time bootstrap invitation emitted to protected logs
  - Later accounts join through administrator-issued invitations
...
- Pairing system: short-lived codes exchanged for server ID + API key
  ...
  - `sc-server pair <CODE> --sc-web-url <URL>` CLI command
- Host runtime (`sc-server`): polls gateway, runs libretro cores in-process, streams WebRTC media
- Gateway (`sc-web`): Next.js 15 web UI, library management, command relay, session records, room/sharing
...
- Build/release scripts (`scripts/build-release.sh`, `scripts/deploy-sc-web.sh`, `scripts/deploy-dev.sh`)
...
- Architecture consolidated from three binaries (`sc-web` + `sc-server` + `sc-worker`) to two roles (`sc-web` + `sc-server`)
  - Emulator/runtime path merged into `sc-server`; no separate `sc-worker` process
...
- Project documentation restructured:
  ...
  - Removed obsolete architecture-history docs: `docs/plans/`, `docs/adr/`, `docs/architecture.html`, `docs/sc-worker-api.md`, `AUDIT_REPORT.md`
- CI workflow updated: `sc-worker` references replaced with the current workspace
...
- Runtime temp directory moved from `/tmp/sc-workers` to `/tmp/sc-sessions`
- `source` field in launch events renamed from `"sc-worker"` to `"host-runtime"`
...
- Separate `sc-worker` crate and binary — runtime is in-process inside `sc-server`
...
- Hardcoded test-domain defaults (`sprite-cloud.com`, `vault:3000`)
...
- Stale `deploy-vault.sh`, `deploy-vps-web.sh`, `promote-main.sh`, `sc-web-cleanup.sh` scripts

### Fixed

- `sc-worker` crate test references in Rust test code
- Docker container naming drift (`sc-web-prod` → `sc-web`)
