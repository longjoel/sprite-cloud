...
const scenarios: MultiplayerVerificationScenario[] = [
  ...
  },
  {
    ...
    logGuidance: {
      ...
      scServer: [
        "Check host_start / guest_offer stage logs and whether sc-server reports fresh PC swaps or missing sessions.",
      ],
      coturn: [
  ...
  {
    id: "same-lan-as-sc-server",
    title: "Scenario 3 — both players on the same LAN as the sc-server host",
    whyItExists:
      "This is the scenario most likely to regress when transport-selection logic or local-route assumptions change.",
    automation: [],
    manualProcedure: [
      "Run host and guest from devices on the same LAN as sc-server and use the normal public site + guest link flow.",
