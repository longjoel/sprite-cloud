...
describe("player option hierarchy", () => {
    ...
    await act(async () => {
      root.render(
        <OptionsOverlay
          ...
          roomRelevant={withRoom}
          audioMuted={false}
          onToggleAudio={actions.mute}
  ...
  it("shows multiplayer group only when room is relevant", async () => {
    await renderOptions(false);
    expect(button("Room")).toBeUndefined();

    await renderOptions(true);
    expect(button("Room")).toBeDefined();
