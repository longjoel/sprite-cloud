...
export default function GamePlayer({
  ...
  useEffect(() => {
    const touchGamepad = window.__scTouchGamepad;
  ...
  const handleLoadAt = (index: number) => {
    ...
    const ok = scPlay.loadStateAt(playerRef.current, index);
    if (!ok) showToast("Not connected", false);
  ...
  const handleListSaves = () => {
    const scPlay = window.scPlay;
    if (!scPlay || !playerRef.current) return;
