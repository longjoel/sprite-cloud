// ── sc-player app — production player glue ───────────────────────────
//
// Imports ScPlayer and wires it to the production player page.
// Handles game start, save/load state commands, reconnect logic.
//
// Loaded from the Next.js player page via <script type="module">.
// Exposes window.scPlay with startPlayer(), saveState(), loadState().

import { ScPlayer, State } from "./sc-player.js";
...
async function startGame(serverId, gameId, corePath, hostToken, callbacks, sdpOffer) {
  ...
  // If we included an SDP offer, sc-web long-polls and returns the answer
...
/**
 * Start a game, create a ScPlayer, connect via relay, and wire callbacks.
 *
...
function startPlayer(video, serverId, gameId, corePath, callbacks, joinToken, hostTokenParam) {
  ...
  player = new ScPlayer(video);  // temp, gets effective ICE config patched before PC creation
  console.log("[gv] ScPlayer created, calling doConnect");
  ...
  try {
    if (window.TouchGamepad) {
      ...
      var preset = video.dataset.scPreset || 'nes';
      var layout = video.dataset.scLayout || 'auto';
      ...
      var isTouchFirst = hasTouch && window.matchMedia('(pointer: coarse)').matches;
      var shouldShow = isTouchFirst;
      ...
      window.__scTouchGamepad = _touchGamepad;
...
window.scPlay = { startPlayer, saveState, loadState, loadStateAt, listSaves };
