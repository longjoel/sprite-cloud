// ── sc-player — browser-side WebRTC client ────────────────────────────
//
// Connects to the sc-server host runtime, negotiates a WebRTC session, and renders
 ...
 *
 * Override by passing `gamepadMapping` in ScPlayer constructor
...
export class ScPlayer {
  ...
  constructor(video, options) {
    if (!video || !video.tagName || video.tagName !== "VIDEO") {
      throw new TypeError("ScPlayer requires a <video> element");
    }
    this._video = video;
      ...
      case "error":
        {
          ...
          console.error("[sc-player] Fatal:", reason);
  ...
  /**
   * Accept synthetic gamepad state from a virtual input device (touch
   * gamepad overlay, on-screen controls, etc.) and forward it to the
   * server using the same DC bitmask format as keyboard + physical
   * gamepad input.
   *
