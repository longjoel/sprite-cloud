...
WEB_DIR="$PROJECT_DIR/sc-web"

VPS_HOST="${GV_VPS_HOST:?set GV_VPS_HOST to the deployment host}"
VPS_USER="${GV_VPS_USER:?set GV_VPS_USER to the deployment account}"
VPS_REPO_DIR="${GV_VPS_REPO_DIR:?set GV_VPS_REPO_DIR to the remote checkout}"
VPS_ENV_FILE="${GV_VPS_ENV_FILE:?set GV_VPS_ENV_FILE to the remote web environment file}"
GV_WEB_URL="${GV_WEB_URL:-https://sprite-cloud.com}"
...
ssh "$VPS_USER@$VPS_HOST" "cd '$VPS_REPO_DIR' && docker build -f docker/sc-web/Dockerfile.prod -t '$IMAGE' --build-arg GV_WEB_GIT_SHA='$GV_SHORT_SHA' --build-arg GV_WEB_GIT_BRANCH='$GV_BRANCH' --build-arg GV_WEB_RELEASED_AT_UTC='$GV_BUILT_AT' ."

log "restarting production container via VPS helper"
ssh "$VPS_USER@$VPS_HOST" "cd '$VPS_REPO_DIR' && GV_WEB_ENV_FILE='$VPS_ENV_FILE' GV_WEB_CONTAINER='$CONTAINER' GV_WEB_IMAGE='$IMAGE' GV_WEB_PUBLIC_ORIGIN='$GV_WEB_PUBLIC_ORIGIN' bash ./deploy-sc-web.sh"
