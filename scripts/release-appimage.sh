...
# Read version from sc-desktop/Cargo.toml or tauri.conf.json.
VERSION="$(awk -F\" '/^version/ {print $2; exit}' "$PROJECT_DIR/sc-desktop/Cargo.toml")"
if [ -z "$VERSION" ]; then
  fail "could not determine version from Cargo.toml"
fi

# Copy to repo root with versioned filename.
RELEASE_NAME="SpriteCloud-${VERSION}.AppImage"
RELEASE_PATH="$PROJECT_DIR/$RELEASE_NAME"
cp "$APPIMAGE" "$RELEASE_PATH"
log "copied AppImage to $RELEASE_PATH"

# Generate SHA256 checksum.
CHECKSUMS_PATH="$PROJECT_DIR/SpriteCloud-${VERSION}.sha256"
