...
# ── apply-gv-web-migration.sh ──────────────────────────────────────────
# Apply a single Drizzle SQL migration to the production Postgres on VPS.
#
# Usage:
#   scripts/apply-gv-web-migration.sh gv-web/drizzle/0012_some_migration.sql
#
# What it does:
#   1. Validates the migration file exists and is readable
#   2. Applies it via psql inside the gv-web-postgres-1 container
#   3. Fails on any SQL error (ON_ERROR_STOP=1)
#   4. Runs a lightweight verification query (checks tables exist)
#
# The migration must be applied BEFORE deploying the new gv-web code.
...
PG_CONTAINER="${GV_PG_CONTAINER:-gv-web-postgres-1}"
PG_USER="${GV_PG_USER:-games_vault}"
PG_DB="${GV_PG_DB:-games_vault}"
...
log "  2. Deploy gv-web: scripts/deploy-gv-web.sh"
log "  3. Verify health:  curl -s https://lngnckr.tech/api/health"
