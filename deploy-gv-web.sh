...
ENV_FILE="${GV_WEB_ENV_FILE:-/root/games-vault/.env}"
CONTAINER="${GV_WEB_CONTAINER:-gv-web-gv-web-1}"
IMAGE="${GV_WEB_IMAGE:-gv-web-prod:latest}"
POSTGRES_CONTAINER="${GV_WEB_POSTGRES_CONTAINER:-gv-web-postgres-1}"
PUBLIC_ORIGIN="${GV_WEB_PUBLIC_ORIGIN:-https://lngnckr.tech}"
...
for item in env:
    ...
    k, v = item.split('=', 1)
    vals[k] = v
user = vals.get('POSTGRES_USER') or 'games_vault'
