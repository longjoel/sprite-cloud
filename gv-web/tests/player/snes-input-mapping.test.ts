...
import { GvPlayer as CanonicalGvPlayer } from "../../public/player/gv-player.js";
import { GvPlayer as PublicGvPlayer } from "../../public/player/index.js";
...
describe("public player entry point", () => {
  it("re-exports the canonical player instead of carrying a stale input implementation", () => {
    expect(PublicGvPlayer).toBe(CanonicalGvPlayer);
  });

  it("has no obsolete server-side duplicate player bundle", () => {
    const repo = fileURLToPath(new URL("../../..", import.meta.url));
    expect(existsSync(`${repo}/gv-server/src/player_bundle.js`)).toBe(false);
    expect(readFileSync(`${repo}/gv-server/src/player_server.rs`, "utf8")).not.toContain("include_str!(\"player_bundle.js\")");
  });
});
