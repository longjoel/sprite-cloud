...
- gv-web: `/api/server/command`, `/api/room/join`, `/api/server/notify` logs with shared command/session IDs.
- gv-server: `[SIGNAL] flow=host_start|guest_offer|host_reconnect` logs.
...
- gv-web: room join, command insert, notify, and SDP-answer resolution logs.
- gv-server: host_start / guest_offer stage logs and reconnect warnings.
- coturn: allocation/auth logs if relay is selected or should have been selected.

### 3) Same LAN as gv-server host

**Goal**
- Prove local-network operation works without the removed gateway-side LAN IP heuristic.

**Manual procedure**
- Use two devices on the same LAN as gv-server.
...
- gv-web: signaling-stage logs showing deterministic flow selection.
- gv-server: session/SDP stage logs for direct-path success or churn.
...
- gv-web: full signaling-stage sequence and any timeout/missing-answer logs.
- gv-server: host_start / guest_offer / host_reconnect logs and SDP warnings.
