...
describe("buildLanPlayerLaunchUrl", () => {
  ...
  it("builds a LAN player URL with host token as query param for proxy forwarding", () => {
    const url = buildLanPlayerLaunchUrl({
      playerUrls: ["http://192.0.2.128:8787/"],
    ...
    expect(url).toBe(
      "http://192.0.2.128:8787/pokemon-yellow?code=ABC123&server_id=server-bazzite&route=lan&host_token=host-secret",
    );
  ...
  it("encodes game ids in the path", () => {
    ...
    expect(url).toBe(
      "http://192.0.2.128:8787/Game%20Boy%2FPok%C3%A9mon%20Yellow.gb?code=XYZ789&server_id=server-vault&route=lan&host_token=host-secret",
    );
  });
