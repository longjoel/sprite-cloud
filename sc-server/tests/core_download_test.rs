...
async fn ensure_core_skips_when_cached() {
    ...
    let result = sc_server::core_bridge::ensure_core("fake_libretro.so", &client).await;
...
async fn ensure_core_download_fails_for_unknown_core() {
    ...
    let client = reqwest::Client::new();
    let result =
        sc_server::core_bridge::ensure_core("definitely_not_a_real_core_libretro.so", &client)
            .await;
