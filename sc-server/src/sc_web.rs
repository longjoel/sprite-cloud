...
/// Non-secret metadata reported by sc-server during verify.
...
/// Non-secret LAN discovery data reported by sc-server.
#[derive(Debug, Serialize)]
...
impl ScWebClient {
    ...
    /// Useful for health checks and other non-sc-web HTTP calls.
    ...
    pub async fn claim(code: &str, sc_web_url: &str, hostname: &str) -> Result<ClaimResponse> {
        let client = reqwest::Client::builder()
            ...
            .build()
            .expect("create HTTP client");
        let url = format!("{}/api/auth/pair/claim", sc_web_url.trim_end_matches('/'));
    ...
    ///
    /// Called after `start_game` starts a host-runtime session so sc-web can
    ...
    ///
    /// Tells sc-web to mark the session as ended so the browser stops
    ...
    /// Notify sc-web that a worker died unexpectedly (crash, OOM, etc.).
