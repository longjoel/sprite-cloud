//! Server metadata collection for sc-web pairing.
//!
//! Collects version and connectivity metadata at startup
//! for reporting to sc-web during server verification.
...
pub(crate) async fn collect_metadata(cfg: &config::Config) -> sc_web::ServerMetadata {
    ...
    let server_version = sc_web::ComponentVersion {
    ...
    let versions = sc_web::VersionMetadata {
        server: server_version,
        worker: sc_web::ComponentVersion {
        ...
        runner: sc_web::ComponentVersion {
    ...
    let interfaces: Vec<sc_web::InterfaceInfo> = if_addrs::get_if_addrs()
        .ok()
        .into_iter()
        .flatten()
        .filter(|iface| !iface.is_loopback())
        .filter(|iface| iface.addr.ip().is_ipv4())
        .map(|iface| sc_web::InterfaceInfo {
            name: iface.name,
    ...
    let runtime = sc_web::RuntimeMetadata {
...
mod tests {
    ...
    fn lan_metadata_builds_player_and_health_urls_for_each_interface() {
        ...
        assert_eq!(
            lan.player_urls,
            vec![
                "http://192.0.2.128:8787/".to_string(),
        ...
        assert_eq!(
            lan.health_urls,
            vec![
                "http://192.0.2.128:8787/health".to_string(),
