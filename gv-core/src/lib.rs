//! Shared-memory IPC between gv-server and gv-core child process.
//!
//! Two mmap'd files in /dev/shm:
//!   gv-out-<game_id>  — core writes frames, server reads
//!   gv-in-<game_id>   — server writes commands, core reads
//!
...
/// Name should be unique per game session (e.g. "gv-out-<game_id>").
