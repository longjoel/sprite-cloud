...
if (typeof window !== "undefined" && typeof document !== "undefined") {
  ...
  let workerParam = params.get("worker");

  // When served from the worker binary itself (no query param), connect to origin.
  // This makes the single-binary self-hosting story work: the worker serves
  // the player UI at /, and the player auto-connects to the same origin.
  if (!workerParam) {
    workerParam = location.origin;
  }
