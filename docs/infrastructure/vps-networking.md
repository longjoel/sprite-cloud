...
> - `/root/.hermes/secrets/vps-<turn-server-ip>.env` on the VAULT host machine (local SSH credential storage)
...
| `/root/.hermes/secrets/vps-<turn-server-ip>.env` | SSH credentials (VAULT host only) |
